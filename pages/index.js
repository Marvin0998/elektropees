...
function isWorkday(dateStr) { const dow=new Date(dateStr).getDay(); return dow>=1&&dow<=4 }
// Pause wird NICHT abgezogen — sie ist bezahlt und zählt zur Arbeitszeit
function calcDauer(start,end) {
  ...
  return (
    <div className="login-page">
      <div style={{textAlign:'center',marginBottom:'2rem'}}>
        ...
        <div className="logo-circle" style={{display:'none'}}>EP</div>
...
function HomePage({user,stunden,baustellen,onStunden}) {
  ...
  // Nur Mo-Do für Regelstunden-Vergleich
  const wocheStunden=myStunden.filter(s=>{const d=new Date(s.datum);return d>=weekStart&&d<=weekEnd&&isWorkday(s.datum)}).reduce((a,s)=>a+s.dauer,0)
  // Freitag separat anzeigen
  const freitagStunden=myStunden.filter(s=>{const d=new Date(s.datum);return d>=weekStart&&d<=weekEnd&&new Date(s.datum).getDay()===5}).reduce((a,s)=>a+s.dauer,0)
  ...
  return (
    <div className="page-content">
      ...
      <button className="hero-btn" onClick={onStunden}>
        <IconClock />
        <div><h3>Stunden erfassen</h3><p>Arbeitszeit für heute eintragen</p></div>
      </button>
      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-num">{wocheStunden.toFixed(1)}</div>
          <div className="stat-label">Std. Mo–Do diese Woche</div>
      ...
      {freitagStunden>0&&(
        <div className="card" style={{background:'#fffbeb',border:'1px solid #f6e05e',marginBottom:'0.75rem'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span style={{fontSize:'0.85rem',color:'#744210'}}>🟡 Freitag diese Woche</span>
            <span style={{fontWeight:700,color:'#744210'}}>{freitagStunden.toFixed(1)} Std</span>
          </div>
        </div>
      )}
      <div className="card">
        ...
        {recent.length===0?<p className="text-muted text-sm">Noch keine Einträge.</p>:recent.map(s=>{
          ...
          const dow=new Date(s.datum).getDay()
          const isFri=dow===5
          return (
    ...
    <div className="page-content">
      ...
      <div className="tab-row">
        {['aktiv','abgeschlossen'].map(f=>(
          <button key={f} className={`tab-btn ${filter===f?'active':''}`} onClick={()=>setFilter(f)}>{f==='aktiv'?'Aktiv':'Archiv'}</button>
        ))}
      </div>
        ...
        <div className="modal-overlay open">
          <div className="modal-sheet">
            ...
            <div className="card" style={{padding:0,overflow:'hidden',marginBottom:'0.75rem'}}>
              {detailStunden.length===0?<p className="text-muted text-sm" style={{padding:'1rem'}}>Noch keine Stunden.</p>:detailStunden.sort((a,b)=>b.datum.localeCompare(a.datum)).map(s=>(
...
function ProfilPage({user,stunden,baustellen}) {
  ...
  const weekStart=getWeekStart(now)
  const weekEnd=new Date(weekStart); weekEnd.setDate(weekEnd.getDate()+6)
  const woche=myStunden.filter(s=>{const d=new Date(s.datum);return d>=weekStart&&d<=weekEnd&&isWorkday(s.datum)}).reduce((a,s)=>a+s.dauer,0)
    ...
    <div className="page-content">
      <div className="card">
        ...
        <div className="stats-row" style={{marginBottom:0}}>
          ...
          <div className="stat-card"><div className="stat-num">{woche.toFixed(1)}</div><div className="stat-label">Std. Mo–Do Woche</div></div>
...
function AdminPage({stunden,baustellen,allUsers}) {
  ...
  async function handleNewUser() {
    ...
    if(data.user){
      await supabase.from('profiles').upsert({id:data.user.id,name:newUser.name,email:newUser.email,role:'mitarbeiter',regel_stunden:newUser.regel_stunden,urlaub_gesamt:newUser.urlaub_gesamt,urlaub_genommen:0})
    }
      ...
      {tab==='mitarbeiter'&&(
        <>
          ...
          {mitarbeiter.map(u=>{
            ...
            const woche=stunden.filter(s=>s.user_id===u.id).filter(s=>{const d=new Date(s.datum);return d>=ws&&d<=we&&isWorkday(s.datum)}).reduce((a,s)=>a+s.dauer,0)
            const diff=woche-(u.regel_stunden||38)
            return (
              <div key={u.id} className="card">
                <div style={{display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'0.75rem'}}>
                  <div className="employee-avatar">{initials(u.name||u.email)}</div>
                  <div><div className="font-bold" style={{color:'#0A0A44'}}>{u.name}</div><div className="text-xs text-muted">{u.email}</div></div>
                </div>
...
function StundenModal({user,baustellen,onClose,onSaved}) {
  ...
  async function handleSave() {
    ...
    await supabase.from('stunden').insert([{
      user_id:user.id,baustelle_id:form.baustelle_id,datum:form.datum,
      start_zeit:form.start,end_zeit:form.end,pause_min:45,dauer,notiz:form.notiz
    }])
  ...
  return (
    <div className="modal-overlay open">
      <div className="modal-sheet">
        ...
        {isFriday&&(
          <div style={{background:'#fef3c7',border:'1px solid #f6e05e',borderRadius:10,padding:'0.6rem 1rem',marginBottom:'1rem',fontSize:'0.82rem',color:'#92400e'}}>
            🟡 <strong>Freitag</strong> — freiwilliger Arbeitstag, wird separat ausgewiesen
          </div>
        )}
        ...
        <div className="card" style={{background:'#f0f4f8',marginBottom:'0.75rem',padding:'0.75rem 1rem'}}>
          ...
          <div style={{fontSize:'0.75rem',color:'#718096',marginTop:4}}>
            ✓ Pause ist bezahlt und zählt zur Arbeitszeit
          </div>
        </div>
  ...
  return (
    <div className="app-container">
      ...
      <nav className="bottom-nav">
        ...
        {isAdmin&&<button className={`nav-item ${page==='admin'?'active':''}`} onClick={()=>setPage('admin')}><IconStar/><span>Admin</span></button>}
      </nav>
    </div>
  )
}
